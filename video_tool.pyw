...
class FFmpegUltimateTool:
    def __init__(self, root):
        ...
        # === 变量定义 (批量混响专属) ===
        self.rv_audio_in = tk.StringVar()
        self.rv_sub_in = tk.StringVar()
        self.rv_out_dir = tk.StringVar()
        self.rv_fmt_var = tk.StringVar(value="WAV")
        self.rv_fmt_options = ["WAV", "MP3", "AAC", "FLAC", "M4A"]
        
        self.rv_offset_head = tk.StringVar(value="200") # 头部偏移(ms)
        self.rv_offset_tail = tk.StringVar(value="200") # 尾部偏移(ms)
        
        # 升级为 freeverb 专业空间混响参数 (默认映射为"大厅混响")
        self.rv_size = tk.StringVar(value="0.8")   # 房间大小 (0-1)
        self.rv_damp = tk.StringVar(value="0.5")   # 高频阻尼/吸收 (0-1)
        self.rv_dry = tk.StringVar(value="0.8")    # 干声增益 (0-1)
        self.rv_wet = tk.StringVar(value="0.4")    # 湿声增益 (0-1)
        self.rv_width = tk.StringVar(value="0.8")  # 立体声宽度 (0-1)
        self.rv_preset_name = tk.StringVar(value="大厅混响 (Great Hall) [默认]")
        self.rv_presets_file = "reverb_presets.json"
        self.rv_presets = {}
        
        self.rv_rules = [] # 存储字幕匹配规则的列表
        self.rv_rule_logic = tk.IntVar(value=1) # 1: 满足任意(OR), 2: 满足所有(AND)
        self.rv_rule_field = tk.StringVar(value="字幕文本 (Text)") # 修改: 适配多列
        self.rv_rule_mode = tk.StringVar(value="包含")
        self.rv_rule_val = tk.StringVar()
        
        self.rv_progress_var = tk.DoubleVar(value=0)
        self.rv_status_text = tk.StringVar(value="等待开始...")
        self.is_rv_processing = False

        # === 注册新标签页：批量添加混响 ===
        self.tab_reverb = ttk.Frame(self.notebook)
        self.notebook.add(self.tab_reverb, text=" 批量添加混响 ")
        self.setup_reverb_ui()
    ...
    def setup_merge_ui(self):
        ...
        ttk.Separator(lf_left, orient='horizontal').grid(row=5, column=0, columnspan=2, sticky="we", pady=5)
        ttk.Label(lf_left, text="(ASS格式自动保留原生样式)", foreground="#888").grid(row=6, column=0, columnspan=2, sticky="w")
        
        ttk.Label(lf_left, text="SRT字体:").grid(row=7, column=0, sticky="w", pady=2)
        
        # 将字体输入框改为组合选项 (加宽下拉框 + 浏览文件按钮)
        frame_font = ttk.Frame(lf_left)
        frame_font.grid(row=7, column=1, sticky="w")
        ...
        ttk.Label(lf_left, text="SRT大小:").grid(row=8, column=0, sticky="w", pady=2)
        ttk.Spinbox(lf_left, from_=10, to=100, textvariable=self.m_font_size, width=10).grid(row=8, column=1, sticky="w")
        
        ttk.Label(lf_left, text="SRT描边:").grid(row=9, column=0, sticky="w", pady=2)
        ttk.Spinbox(lf_left, from_=0, to=10, textvariable=self.m_font_outline, width=10).grid(row=9, column=1, sticky="w")
        
        ttk.Label(lf_left, text="SRT边距:").grid(row=10, column=0, sticky="w", pady=2)
        ttk.Spinbox(lf_left, from_=0, to=100, textvariable=self.m_font_marginv, width=10).grid(row=10, column=1, sticky="w")
        ...
        for i, v_filename in enumerate(video_files):
            ...
            if num_ext_audio > 0:
                if active_in_filter_count == 1:
                    # 只有一条外部音轨，且没有勾选保留原音轨混音
                    idx = f"{v_idx}:a:0" if has_a1 else f"{b_idx}:a:0"
                    vol = vol_v if has_a1 else vol_b
                    
                    if mode == 3: # 选项3: 保持原始单轨(无损拷贝)
                        a_out = idx
                    else: # 选项1或2: 降级为手动音量应用(取消loudnorm防止单轨过度放大产生底噪)
                        fc_parts.append(f"[{idx}]volume={vol}[aout]")
                        a_out = "[aout]"
                    ...
                    if has_a1:
                        fc_parts.append(f"[{v_idx}:a:0]volume={vol_v}[a1_vol]")
                        mix_inputs.append("[a1_vol]")
                    if has_a2:
                        fc_parts.append(f"[{b_idx}:a:0]volume={vol_b}[a2_vol]")
                        mix_inputs.append("[a2_vol]")
                        
                    mix_str = "".join(mix_inputs)
                    
                    if mode == 2: # 自动 loudnorm 平衡
                        fc_parts.append(f"{mix_str}amix=inputs={active_in_filter_count}:duration=longest[amix]")
                        fc_parts.append(f"[amix]loudnorm[aout]")
                        a_out = "[aout]"
                    else: # 手动音量混合 (即使选了选项3，对于多轨也智能回退到此模式)
                        fc_parts.append(f"{mix_str}amix=inputs={active_in_filter_count}:duration=longest[aout]")
                        a_out = "[aout]"
...
# ================= 新增：批量添加混响 UI 与逻辑 =================
    def setup_reverb_ui(self):
        # 1. 目录设置区
        frame_dir = ttk.LabelFrame(self.tab_reverb, text="输入输出目录", padding=10)
        frame_dir.pack(fill="x", padx=10, pady=5)
        frame_dir.columnconfigure(1, weight=1)

        ttk.Label(frame_dir, text="音/视频输入:").grid(row=0, column=0, sticky="e", pady=2)
        ttk.Entry(frame_dir, textvariable=self.rv_audio_in).grid(row=0, column=1, sticky="we", padx=5)
        ttk.Button(frame_dir, text="浏览...", command=lambda: self.browse_dir(self.rv_audio_in)).grid(row=0, column=2)

        ttk.Label(frame_dir, text="字幕目录:").grid(row=1, column=0, sticky="e", pady=2)
        ttk.Entry(frame_dir, textvariable=self.rv_sub_in).grid(row=1, column=1, sticky="we", padx=5)
        ttk.Button(frame_dir, text="浏览...", command=lambda: self.browse_dir(self.rv_sub_in)).grid(row=1, column=2)

        ttk.Label(frame_dir, text="音频输出:").grid(row=2, column=0, sticky="e", pady=2)
        ttk.Entry(frame_dir, textvariable=self.rv_out_dir).grid(row=2, column=1, sticky="we", padx=5)
        ttk.Button(frame_dir, text="浏览...", command=lambda: self.browse_dir(self.rv_out_dir)).grid(row=2, column=2)

        # 2. 字幕匹配规则组合区
        frame_rules = ttk.LabelFrame(self.tab_reverb, text="智能字幕条件过滤 (通过内容/正则/角色精确锁定混响时间轴)", padding=10)
        frame_rules.pack(fill="x", padx=10, pady=5)
        
        f_add = ttk.Frame(frame_rules)
        f_add.pack(fill="x", pady=2)
        ttk.Combobox(f_add, textvariable=self.rv_rule_field, values=["字幕文本 (Text)", "样式 (Style)", "角色 (Name)", "特效 (Effect)", "全部列综合"], state="readonly", width=18).pack(side="left", padx=2)
        ttk.Combobox(f_add, textvariable=self.rv_rule_mode, values=["包含", "正则匹配", "等于"], state="readonly", width=10).pack(side="left", padx=2)
        ttk.Entry(f_add, textvariable=self.rv_rule_val, width=25).pack(side="left", padx=5, fill="x", expand=True)
        ttk.Button(f_add, text="添加规则", command=self.add_rv_rule, width=10).pack(side="left", padx=2)

        self.rv_rule_listbox = tk.Listbox(frame_rules, height=3)
        self.rv_rule_listbox.pack(fill="x", pady=5)

        f_logic = ttk.Frame(frame_rules)
        f_logic.pack(fill="x")
        ttk.Radiobutton(f_logic, text="满足 任意 规则即可 (OR)", variable=self.rv_rule_logic, value=1).pack(side="left", padx=(0,15))
        ttk.Radiobutton(f_logic, text="满足 所有 规则 (AND)", variable=self.rv_rule_logic, value=2).pack(side="left")
        ttk.Button(f_logic, text="删除选中规则", command=self.delete_rv_rule).pack(side="right")

        # 3. 混响参数与预设区
        frame_params = ttk.LabelFrame(self.tab_reverb, text="混响参数 (aecho) 与预设管理", padding=10)
        frame_params.pack(fill="x", padx=10, pady=5)
        
        f_p1 = ttk.Frame(frame_params)
        f_p1.pack(fill="x", pady=2)
        ttk.Label(f_p1, text="房间大小(0-1):").pack(side="left")
        ttk.Entry(f_p1, textvariable=self.rv_size, width=5).pack(side="left", padx=(2, 10))
        ttk.Label(f_p1, text="高频阻尼(0-1):").pack(side="left")
        ttk.Entry(f_p1, textvariable=self.rv_damp, width=5).pack(side="left", padx=(2, 10))
        ttk.Label(f_p1, text="干声级(0-1):").pack(side="left")
        ttk.Entry(f_p1, textvariable=self.rv_dry, width=5).pack(side="left", padx=(2, 10))
        ttk.Label(f_p1, text="湿声级(0-1):").pack(side="left")
        ttk.Entry(f_p1, textvariable=self.rv_wet, width=5).pack(side="left", padx=(2, 10))
        ttk.Label(f_p1, text="立体宽(0-1):").pack(side="left")
        ttk.Entry(f_p1, textvariable=self.rv_width, width=5).pack(side="left", padx=(2, 0))

        f_p2 = ttk.Frame(frame_params)
        f_p2.pack(fill="x", pady=(8,0))
        ttk.Label(f_p2, text="Audition级预设:").pack(side="left")
        self.cb_rv_preset = ttk.Combobox(f_p2, textvariable=self.rv_preset_name, width=26, state="readonly")
        self.cb_rv_preset.pack(side="left", padx=5)
        ttk.Button(f_p2, text="应用预设", command=self.apply_rv_preset).pack(side="left", padx=2)
        ttk.Button(f_p2, text="保存当前", command=self.save_rv_preset).pack(side="left", padx=2)
        ttk.Button(f_p2, text="删除预设", command=self.delete_rv_preset).pack(side="left", padx=2)
        self.load_rv_presets() # 初始化加载预设

        # 4. 偏移量与格式输出
        frame_out = ttk.LabelFrame(self.tab_reverb, text="时间轴偏移与输出", padding=10)
        frame_out.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(frame_out, text="前置偏移(向外扩宽 ms):").pack(side="left")
        ttk.Entry(frame_out, textvariable=self.rv_offset_head, width=6).pack(side="left", padx=(2, 15))
        ttk.Label(frame_out, text="后置偏移(向外扩宽 ms):").pack(side="left")
        ttk.Entry(frame_out, textvariable=self.rv_offset_tail, width=6).pack(side="left", padx=(2, 20))
        
        ttk.Label(frame_out, text="输出格式:").pack(side="left")
        ttk.Combobox(frame_out, textvariable=self.rv_fmt_var, values=self.rv_fmt_options, state="readonly", width=8).pack(side="left", padx=5)

        # 5. 底部状态与操作
        frame_bottom_rv = ttk.Frame(self.tab_reverb, padding=(10, 10))
        frame_bottom_rv.pack(fill="x", side="bottom", pady=5)
        frame_bottom_rv.columnconfigure(1, weight=1)
        self.lbl_rv_status = ttk.Label(frame_bottom_rv, textvariable=self.rv_status_text)
        self.lbl_rv_status.grid(row=0, column=0, sticky="e", padx=(0, 5))
        self.rv_pb = ttk.Progressbar(frame_bottom_rv, orient="horizontal", mode="determinate", variable=self.rv_progress_var)
        self.rv_pb.grid(row=0, column=1, sticky="we", padx=10)
        self.btn_run_rv = ttk.Button(frame_bottom_rv, text="开始处理", width=12, command=self.start_reverb)
        self.btn_run_rv.grid(row=0, column=2, padx=5)
        self.btn_stop_rv = ttk.Button(frame_bottom_rv, text="停止", width=12, command=self.stop_reverb, state="disabled")
        self.btn_stop_rv.grid(row=0, column=3)

    # --- 规则管理功能 ---
    def add_rv_rule(self):
        v = self.rv_rule_val.get().strip()
        if not v: return
        self.rv_rules.append({
            'field': self.rv_rule_field.get(), 'mode': self.rv_rule_mode.get(), 'val': v
        })
        self.refresh_rv_rule_list()
        self.rv_rule_val.set("")

    def delete_rv_rule(self):
        sel = self.rv_rule_listbox.curselection()
        if sel:
            del self.rv_rules[sel[0]]
            self.refresh_rv_rule_list()

    def refresh_rv_rule_list(self):
        self.rv_rule_listbox.delete(0, tk.END)
        for r in self.rv_rules:
            self.rv_rule_listbox.insert(tk.END, f"[{r['field']}]  {r['mode']} :  {r['val']}")

    def check_rv_rules(self, field_data):
        if not self.rv_rules: return True # 如果没加规则，默认放行
        ...
        for rule in self.rv_rules:
            ...
            if rule['mode'] == '包含': is_match = rule['val'] in target
                ...
                try: 
                    if re.search(rule['val'], target): is_match = True
                except: pass
    ...
    # --- 预设管理功能 ---
    def load_rv_presets(self):
        try:
            if os.path.exists(self.rv_presets_file):
                with open(self.rv_presets_file, 'r', encoding='utf-8') as f:
                    self.rv_presets = json.load(f)
        except: pass
        if not self.rv_presets:
            # 内置对标 Adobe Audition 的经典环境声场预设
            self.rv_presets = {
                "大厅混响 (Great Hall) [默认]": {"size": "0.8", "damp": "0.5", "dry": "0.8", "wet": "0.4", "width": "0.8"},
                "宽阔人声 (Vocal Wide)": {"size": "0.6", "damp": "0.4", "dry": "0.9", "wet": "0.3", "width": "1.0"},
                "小房间/录音室 (Small Room)": {"size": "0.3", "damp": "0.8", "dry": "0.9", "wet": "0.2", "width": "0.4"},
                "大教堂 (Church)": {"size": "0.9", "damp": "0.3", "dry": "0.7", "wet": "0.6", "width": "0.9"},
                "轻微润色 (Subtle)": {"size": "0.2", "damp": "0.5", "dry": "1.0", "wet": "0.15", "width": "0.5"},
                "地下车库 (Garage)": {"size": "0.7", "damp": "0.2", "dry": "0.8", "wet": "0.5", "width": "0.7"}
            }
        self.cb_rv_preset['values'] = list(self.rv_presets.keys())
        self.cb_rv_preset.bind('<<ComboboxSelected>>', self.apply_rv_preset)
        self.apply_rv_preset() # 启动时自动应用一次默认预设的数值

    def save_rv_preset(self):
        name = self.rv_preset_name.get().strip()
        if not name: return
        self.rv_presets[name] = {
            "size": self.rv_size.get(), "damp": self.rv_damp.get(), 
            "dry": self.rv_dry.get(), "wet": self.rv_wet.get(), "width": self.rv_width.get()
        }
        with open(self.rv_presets_file, 'w', encoding='utf-8') as f: json.dump(self.rv_presets, f, ensure_ascii=False)
        self.cb_rv_preset['values'] = list(self.rv_presets.keys())
        messagebox.showinfo("成功", f"预设 '{name}' 已保存！")

    def apply_rv_preset(self, event=None):
        name = self.rv_preset_name.get().strip()
        if name in self.rv_presets:
            p = self.rv_presets[name]
            self.rv_size.set(p.get("size", "0.8"))
            self.rv_damp.set(p.get("damp", "0.5"))
            self.rv_dry.set(p.get("dry", "0.8"))
            self.rv_wet.set(p.get("wet", "0.4"))
            self.rv_width.set(p.get("width", "0.8"))

    def delete_rv_preset(self):
        name = self.rv_preset_name.get().strip()
        if name in self.rv_presets:
            del self.rv_presets[name]
            with open(self.rv_presets_file, 'w', encoding='utf-8') as f: json.dump(self.rv_presets, f, ensure_ascii=False)
            self.cb_rv_preset['values'] = list(self.rv_presets.keys())
            if self.rv_presets:
                self.rv_preset_name.set(list(self.rv_presets.keys())[0])
                self.apply_rv_preset()

    # --- 核心时间轴解析逻辑 ---
    def rv_time_to_sec(self, t_str, is_srt=False):
        try:
            parts = t_str.strip().split(':')
            if is_srt:
                sec_parts = parts[2].split(',')
                return int(parts[0])*3600 + int(parts[1])*60 + int(sec_parts[0]) + int(sec_parts[1])/1000.0
            else:
                sec_parts = parts[2].split('.')
                return int(parts[0])*3600 + int(parts[1])*60 + int(sec_parts[0]) + int(sec_parts[1])/100.0
        except: return 0.0

    def parse_subtitles_for_intervals(self, sub_path):
        if not self.rv_rules: return [] # 无规则直接跳过
        
        try: offset_head = int(self.rv_offset_head.get()) / 1000.0
        except: offset_head = 0.2
        try: offset_tail = int(self.rv_offset_tail.get()) / 1000.0
        except: offset_tail = 0.2

        intervals = []
        is_ass = sub_path.lower().endswith('.ass')
        
        with open(sub_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
            
        if is_ass:
            # 放弃粗略正则，改为根据逗号严格切分 ASS 的 10 大标准列数据
            for line in content.split('\n'):
                line = line.strip()
                if line.startswith("Dialogue:"):
                    # 切除 "Dialogue:" 前缀，并切割出最多 10 个部分，保留最后的正文逗号
                    parts = line.split(":", 1)[1].strip().split(",", 9)
                    if len(parts) == 10:
                        start_str, end_str = parts[1], parts[2]
                        style, name, effect, text = parts[3], parts[4], parts[8], parts[9]
                        ...
                        # 把这行字幕的字典送入引擎进行组合校验
                        if not self.check_rv_rules(field_data): continue
                        
                        start_sec = self.rv_time_to_sec(start_str, is_srt=False)
                        end_sec = self.rv_time_to_sec(end_str, is_srt=False)
                        intervals.append([max(0, start_sec - offset_head), end_sec + offset_tail])
        else:
            # 兼容老版 SRT 格式 (SRT 只有纯文本，没有特效/样式等列)
            blocks = content.split('\n\n')
            for block in blocks:
                lines = block.strip().split('\n')
                if len(lines) >= 3:
                    t_match = re.search(r"(\d{2}:\d{2}:\d{2},\d{3})\s*-->\s*(\d{2}:\d{2}:\d{2},\d{3})", lines[1])
                    if t_match:
                        text = '\n'.join(lines[2:])
                        field_data = {
                            "字幕文本 (Text)": text,
                        ...
                        if not self.check_rv_rules(field_data): continue
                        start_sec, end_sec = self.rv_time_to_sec(t_match.group(1), is_srt=True), self.rv_time_to_sec(t_match.group(2), is_srt=True)
                        intervals.append([max(0, start_sec - offset_head), end_sec + offset_tail])
                        
        # 融合重叠的时间段 (核心安全机制：防止密集排版导致混响滤镜时间轴冲突崩溃)
    ...
    def start_reverb(self):
        if not all([self.rv_audio_in.get(), self.rv_sub_in.get(), self.rv_out_dir.get()]):
            messagebox.showerror("错误", "请完善输入和输出目录设置！")
            return
        if not self.rv_rules:
            messagebox.showwarning("提示", "请至少添加一条字幕过滤规则！")
            return
            
        self.is_rv_processing = True
        self.is_cancelled = False
        self.btn_run_rv.config(state="disabled")
        self.btn_stop_rv.config(state="normal")
        self.rv_progress_var.set(0)
        
        threading.Thread(target=self.process_reverb_thread, daemon=True).start()

    def stop_reverb(self):
        if self.is_rv_processing:
            self.is_cancelled = True
            self.rv_status_text.set("正在中止处理...")
            self.btn_stop_rv.config(state="disabled")
            if self.current_process:
                try: self.current_process.kill()
                except: pass

    def process_reverb_thread(self):
        in_dir = self.rv_audio_in.get().strip()
        sub_dir = self.rv_sub_in.get().strip()
        out_dir = self.rv_out_dir.get().strip()
        os.makedirs(out_dir, exist_ok=True)
        
        all_exts = self.supported_exts + ['.wav', '.mp3', '.aac', '.flac', '.m4a']
        media_files = [f for f in os.listdir(in_dir) if os.path.splitext(f)[1].lower() in all_exts]
        
        processed_count = 0
        fmt = self.rv_fmt_var.get()
        ...
        for f_name in media_files:
            ...
            s_path = None
            for ext in ['.ass', '.srt']:
                temp = os.path.join(sub_dir, base_name + ext)
                if os.path.exists(temp):
                    s_path = temp
                    break
                    
            if not s_path: continue # 没找到字幕，跳过
            
            # 提取混响时间轴区间
            intervals = self.parse_subtitles_for_intervals(s_path)
            
            out_ext = "." + fmt.lower()
            out_path = os.path.join(out_dir, base_name + out_ext)
            
            cmd = [self.ffmpeg_bin, "-y", "-i", in_path]
            temp_filter_file = os.path.join(out_dir, f"temp_rv_filter_{threading.get_ident()}.txt")
            
            if intervals:
                # 动态生成超长局部混响指令并写入临时文件以避开 cmd 字符长度限制
                enable_expr = "+".join([f"between(t,{s:.3f},{e:.3f})" for s, e in intervals])
                # 获取 5 大专业参数
                rs, rd = self.rv_size.get(), self.rv_damp.get()
                dry, wet, width = self.rv_dry.get(), self.rv_wet.get(), self.rv_width.get()
                
                # 使用 freeverb 算法替代 aecho，获得广播级空间混响效果
                filter_content = f"[0:a:0]freeverb=size={rs}:damp={rd}:dry={dry}:wet={wet}:width={width}:enable='{enable_expr}'[aout]"
                with open(temp_filter_file, "w", encoding="utf-8") as f: f.write(filter_content)
                
                cmd.extend(["-filter_complex_script", temp_filter_file.replace('\\', '/'), "-map", "[aout]"])
            else:
                cmd.extend(["-map", "0:a:0"]) # 没有匹配区间，原声直出
                
            # 编码设置
            if fmt == "WAV": cmd.extend(["-c:a", "pcm_s16le"])
            elif fmt == "MP3": cmd.extend(["-c:a", "libmp3lame", "-b:a", "320k"])
            elif fmt in ["AAC", "M4A"]: cmd.extend(["-c:a", "aac", "-b:a", "320k"])
            elif fmt == "FLAC": cmd.extend(["-c:a", "flac"])
            cmd.append(out_path)
            
            # 执行 FFmpeg
            self.root.after(0, self.rv_status_text.set, f"正在处理: {f_name}")
            self.root.after(0, self.rv_progress_var.set, 0)
            total_duration_sec = 0
            try:
                self.current_process = subprocess.Popen(cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE, encoding='utf-8', errors='ignore', creationflags=subprocess.CREATE_NO_WINDOW)
                for line in self.current_process.stderr:
                    ...
                    if time_match and total_duration_sec > 0:
                        current_sec = self.time_to_seconds(*time_match.groups())
                        percent = min((current_sec / total_duration_sec) * 100, 100)
                        self.root.after(0, self.rv_progress_var.set, percent)
                self.current_process.wait()
                processed_count += 1
            except Exception as e:
                if self.current_process:
        ...
        if self.is_cancelled:
            self.root.after(0, self.rv_reset_ui, "批量混响已中止！")
        else:
            self.root.after(0, self.rv_progress_var.set, 100)
            msg = "批量混响处理完毕！" if processed_count > 0 else "未发现符合条件的媒体与字幕对，已跳过。"
            self.root.after(0, self.rv_reset_ui, msg, processed_count > 0)

    def rv_reset_ui(self, message, success=False):
        self.is_rv_processing = False
        self.current_process = None
        self.btn_run_rv.config(state="normal")
        self.btn_stop_rv.config(state="disabled")
        self.rv_status_text.set(message)
        if success: messagebox.showinfo("完成", message)
        else: messagebox.showwarning("提示", message)
